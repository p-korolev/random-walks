import math, random as R, stat_helper as sh, matplotlib.pyplot as plt
...
class Walk():
    ...
    def generate_walk(self, num_steps: int, add_interval: list = [1, 1], 
                      add_probability: float = 0.5, uniform: bool = False):
        ...
        for step in range(num_steps):
            # get +/- decision
            decision = R.uniform(0,1)

            # if interval subset Z
            if (uniform):
                # generate random +/- value
                random_val = R.uniform(add_interval[0], add_interval[1])
            ...
            if (not uniform):
                random_val = R.randint(add_interval[0], add_interval[1])
                if decision<add_probability:
                    self.current_walk.append(self.current_value + random_val)
                    self.current_value = self.current_walk[-1]
                    print(f"Adding {random_val}")
                else:
                    self.current_walk.append(self.current_value - random_val) 
                    self.current_value = self.current_walk[-1]
                    print(f"Subtracting {random_val}")

    # re-generate walk beginning at start value
    def regenerate_walk(self):
    ...
    # plot current walk
    def plot_walk(self, show_sma: bool = False, sma_period: int = None, 
        ...
        if (show_sma):
            ...
            # plot moving average
            plt.plot(sma_X, sma_Y, color = "green", label="Simple Moving Average")
        ...
        if (show_running_variance):
            ...
            # plot running variance
            plt.plot(X, var_list, color="purple", label="Variance")
...
# testing ------------------------------------------------------------------
walk1 = Walk(5)
walk1.generate_walk(10)
print(walk1.get_size())
print(walk1.view_walk())
walk1.plot_walk(show_sma=True, show_running_variance=True, sma_period=2)
